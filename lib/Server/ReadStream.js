...
ReadStream.prototype._transform = function (chunk, encoding, next) {
	...
	while (this.pending.length >= 6) {
		...
		switch (pkg.functionCode) {
			case 'READ_COILS':
				pkg.from = data.readUInt16BE(0) + 1;
				pkg.to = data.readUInt16BE(2) + pkg.from - 1;
				break;
			case 'READ_DISCRETE_INPUTS':
				pkg.from = data.readUInt16BE(0) + 10001;
				pkg.to = data.readUInt16BE(2) + pkg.from - 1;
				break;
			case 'READ_HOLDING_REGISTERS':
				pkg.from = data.readUInt16BE(0) + 40001;
				pkg.to = data.readUInt16BE(2) + pkg.from - 1;
				break;
			case 'READ_INPUT_REGISTERS':
				pkg.from = data.readUInt16BE(0) + 30001;
				pkg.to = data.readUInt16BE(2) + pkg.from - 1;
				break;
			case 'WRITE_SINGLE_COIL':
				pkg.address = data.readUInt16BE(0) + 1;
				pkg.value   = new Buffer([ data[2], data[3] ]);
				break;
			case 'WRITE_SINGLE_REGISTER':
				pkg.address = data.readUInt16BE(0) + 40001;
				pkg.value   = new Buffer([ data[2], data[3] ]);
				break;
